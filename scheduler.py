...
def Ingest(Tag):
    ...
    # 修改这里的本地路径以加载数据源
    file_descriptor = FileDescriptor(r"D:\test\Results\log_data_merge\{}.csv".format(Tag),3333)  # 3333 is the raw size of the data in bytes.
...
if __name__ == '__main__':
    ...
    DROP_TABLE_IF_EXIST = """.drop extents <| .show table RepoContributors extents where tags has 'drop-by:{}'""".format(StartTime)
    RESPONSE = ls[0].execute_mgmt("DevRelWorkArea", DROP_TABLE_IF_EXIST)
    for i in range(398):
        Ingest("{}".format(StartTime))
